...
namespace WatchGuideAPI.Services
{
    ...
    public class ContentService : IContentService
    {
        ...
        public ContentService(AppDbContext context, ITMDBService tmdbService, IWatchmodeService watchmodeService)
        {
            _context = context;
        ...
        private async Task<Content?> GetValidCachedContent(int tmdbId, string mediaType) =>
            await _context.Content
                .Include(c => c.ContentGenres)
                .FirstOrDefaultAsync(c => c.TmdbId == tmdbId && c.Type == mediaType && c.CachedUntil > DateTime.UtcNow);

        private async Task<Content> GetOrCreateContent(int tmdbId, string mediaType, ContentDetailsResponse details)
        {
            var content = await _context.Content
                .Include(c => c.ContentGenres)
                .FirstOrDefaultAsync(c => c.TmdbId == tmdbId && c.Type == mediaType);
        ...
        {
            _context.ContentCast.RemoveRange(_context.ContentCast.Where(c => c.ContentId == contentId));
        ...
        {
            _context.ContentGenres.RemoveRange(_context.ContentGenres.Where(g => g.ContentId == contentId));

            foreach (var g in genres)
                _context.ContentGenres.Add(new ContentGenre { ContentId = contentId, Genre = g });
        }

        private async Task<List<string>> GetUserGenres(Guid userId) =>
            await _context.UserGenrePreferences
